...
public class ProjworkController implements Serializable {
    ...
    @EJB
    private JOW.ProjworkFacade ejbFacade;
    ...
    public Projwork getSelected() {
        if (current == null) {
            ...
            current.setProjworkPK(new JOW.ProjworkPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public Projwork getProjwork(JOW.ProjworkPK id) {
    ...
    public static class ProjworkControllerConverter implements Converter {
        ...
        JOW.ProjworkPK getKey(String value) {
            JOW.ProjworkPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new JOW.ProjworkPK();
            key.setProjno(values[0]);
            key.setEmpnum(values[1]);
            return key;
        }

        String getStringKey(JOW.ProjworkPK value) {
